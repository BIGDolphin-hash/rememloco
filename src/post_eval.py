...
from src.dataset_info import MPDD_OBJECT_ANOMALIES, sorted_image_paths
...
def parse_dataset_files(object_name, dataset_base_dir, anomaly_maps_dir, dataset="MVTec"):
    ...
    for subdir in sorted(listdir(str(test_dir))):
        ...
        test_images = [image_path.stem for image_path in sorted_image_paths(subdir_path)]
        ...
        if subdir != 'good':
            if dataset in {'MVTec', 'MPDD'}:
                ...
                ground_truth_images = sorted_image_paths(
                    path.join(gt_base_dir, subdir)
                )
                if not ground_truth_images:
                    raise FileNotFoundError(
                        f"No ground-truth masks found in "
                        f"{path.join(gt_base_dir, subdir)}"
                    )
                mask_type = ground_truth_images[0].suffix
                gt_filenames.extend(
                    [path.join(gt_base_dir, subdir, file + mask_type)
                    for file in test_images])
        else:
...
def read_tiff(file_path_no_ext, exts=('.tif', '.tiff', '.TIF', '.TIFF')):
    ...
    Raises:
        ...
        IOError: The given file path exists with multiple of the given
          extensions.
    """
    # Inspect actual directory entries instead of probing case variants. On a
    # case-insensitive filesystem, probing both '.tiff' and '.TIFF' identifies
    # the same file twice.
    base_path = Path(file_path_no_ext)
    ...
    if base_path.parent.is_dir():
        file_paths = [
            ...
            and candidate.stem == base_path.name
...
def eval_segmentation(gt_filenames, prediction_filenames, pro_integration_limit=0.3, delete_tiff_files=True):
    ...
    ap_px = average_precision_score(ground_truth, predictions)
    print(f"AP (pixel-level): {ap_px}", end=" -- ")
    ...
    return au_pro, auroc_px, f1_px, ap_px


def _resolve_test_image_path(dataset_base_dir, object_name, prediction_name):
    if dataset_base_dir is None or object_name is None:
        raise ValueError(
            "dataset_base_dir and object_name are required to locate source "
            "test images"
        )

    prediction_path = Path(prediction_name)
    test_type = prediction_path.parent.name
    image_stem = prediction_path.name
    test_dir = Path(dataset_base_dir) / object_name / "test" / test_type
    matches = [
        image_path
        for image_path in sorted_image_paths(test_dir)
        if image_path.stem == image_stem
    ]
    if len(matches) != 1:
        raise FileNotFoundError(
            "Expected exactly one source test image for anomaly map "
            f"{prediction_name}, found {len(matches)} in {test_dir}"
        )
    return matches[0]
...
def eval_classification(
    ...
    for (gt_name, pred_name) in tqdm(zip(gt_filenames, prediction_filenames),
        ...
        if aggregation_statistics == "max_anomaly_map" or anomaly_scores is not None:
            source_image_path = _resolve_test_image_path(
                dataset_base_dir,
                object_name,
                pred_name,
            )
        sample_keys.append(
            ...
            else f"{source_image_path.parent.name}/{source_image_path.name}"
        )
        if First_flag:
            First_flag = False
            if os.path.exists(pred_name +'_class' + '.npy'):
                Flag = True
        if Flag:
            prediction = np.load(pred_name + '_class' + '.npy')
        else:
            prediction = np.load(pred_name + '.npy') 
        predictions.append(prediction)
        # if results is None:
        if aggregation_statistics == "max_anomaly_map":
            with Image.open(source_image_path) as image_test:
                gt_img_size.append((image_test.height, image_test.width))
    ...
    if anomaly_scores is not None:
        for idx, sample_key in enumerate(sample_keys):
            predictions[idx] = anomaly_scores[sample_key]
...
def get_objects_from_dataset(dataset):
    ...
    if dataset == "MVTec":
        ...
        objects = list(MPDD_OBJECT_ANOMALIES)
    else:
        raise ValueError(
            f"Unknown dataset: {dataset}. Expected one of: MVTec, VisA, MPDD."
        )
    return objects
    

def eval_finished_run(dataset, dataset_base_dir, anomaly_maps_dir, output_dir, seed = None, object_name=None, pro_integration_limit = 0.3, eval_clf = True, eval_segm = False, delete_tiff_files = True, aggregation_statistics = "meantop1p", anomaly_scores=None, results=None):
    """
    Evaluate the results of a finished run on the MVTec AsD dataset.
    Arguments:
    ...
    all_objects = get_objects_from_dataset(dataset)
    if object_name is None:
        objects = all_objects
    elif isinstance(object_name, str):
        objects = [object_name]
    else:
        objects = list(object_name)
    unknown_objects = sorted(set(objects) - set(all_objects))
    if unknown_objects:
        raise ValueError(
            f"Unknown {dataset} objects requested for evaluation: {unknown_objects}"
        )
    ...
    for obj in objects:
        ...
        # Evaluate classification performance
            auroc_clf, ap_clf, f1_clf = \
                eval_classification(
                    ...
                    anomaly_scores=anomaly_scores[obj] if anomaly_scores is not None else None,
                    dataset_base_dir=dataset_base_dir,
                    object_name=obj)
        ...
        if eval_segm:
            ...
            evaluation_dict[obj]['seg_AP'] = ap_px
            if results is not None:
                evaluation_dict[obj]['ref_images'] = results[obj]['img_ref_samples']
            # Keep track of the mean performance measures.
    ...
    if eval_segm:
        ...
        evaluation_dict['mean_segmentation_ap'] = np.mean(ap_px_ls).item()
    if eval_clf:
    ...
    return path.join(output_dir, metric_file)
